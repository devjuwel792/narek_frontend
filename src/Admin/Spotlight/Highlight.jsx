...
import {
  ...
  useGetContentsQuery,
} from "../../Redux/services/highlightApi";
  ...
  useEffect(() => {
    if (highlightsData?.data) {
      const mappedCards = highlightsData?.data?.results.map((item) => ({
        ...
        views: item.content.views_count,
        image:
          item.content.thumbnail_image ||
          "https://images.pexels.com/photos/3109807/pexels-photo-3109807.jpeg?auto=compress&cs=tinysrgb&w=400", // placeholder image
        highlighted: true,
      }));
  ...
  return (
    <div className="">
            ...
            >
              ...
              <div className="flex gap-5 py-4 ">
                ...
                {/* <button
                  ...
                  className={`flex-1 px-4 py-2 rounded transition-colors text-sm font-medium bg-[#c8ff00] text-purple-950 hover:bg-[#b3e600]"
                    `}
                >
                  Update Highlight
                </button> */}
      ...
      <Modal open={open} className="max-w-4xl  p-4 " onOpenChange={setOpen}>
        <SelectContent setOpen={setOpen} addHighlight={addHighlight} />
      </Modal>
    </div>
  );
}
...
function SelectContent({ setOpen, addHighlight }) {
  ...
  const getTypeColor = (type) => {
    const colors = {
      Video: "bg-pink-600/20 text-pink-600",
      Blog: "bg-yellow-600/20 text-yellow-600",
      PDF: "bg-red-600/20 text-red-600",
      Image: "bg-purple-600/20 text-purple-600",
    };
