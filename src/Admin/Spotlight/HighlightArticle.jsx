...
import {
  useCreateHighlightArticleMutation,
  useDeleteHighlightArticleMutation,
  useGetHighlightArticlesQuery,
} from "../../Redux/services/highlightArticleApi";
...
export default function HighlightArticle() {
  const { data: highlightsData, isLoading, error } = useGetHighlightArticlesQuery();
  const [createHighlight] = useCreateHighlightArticleMutation();
  const [deleteHighlight] = useDeleteHighlightArticleMutation();
  ...
  useEffect(() => {
    if (highlightsData?.data) {
      const mappedCards = highlightsData?.data?.results.map((item) => ({
        ...
        type: "Article", // Assuming all are articles
        views: item.article.like_count, // Using like_count as views
        image: item.article.banner_image,
        highlighted: true,
      }));
  ...
  return (
    <div className="">
      ...
      <Modal open={open} className="max-w-4xl  p-4 " onOpenChange={setOpen}>
        <SelectArticle setOpen={setOpen} addHighlight={addHighlight} />
      </Modal>
    </div>
  );
}

function SelectArticle({ setOpen, addHighlight }) {
  const {
    data: articlesData,
    isLoading,
    error,
  } = useGetArticlesQuery({ search: "" });
  ...
  const data =
    articlesData?.data?.results?.map((item) => ({
      id: item.id,
      title: item.title,
